...
//         p[id].pos.xyz = vec3(emitterX, emitterY, emitterZ);
//         p2[id].pos.xyz = vec3(emitterX, emitterY, emitterZ);
//         p[id].vel.x = rand(-15, 15, gl_GlobalInvocationID.x * 7.0);; // or random velocity if you want
//         p[id].vel.y = 10.0;
//         p[id].vel.z = rand(-15, 15, gl_GlobalInvocationID.x * 11.0);
//         p[id].pos.w = maxLifeTime;
//     } else {
//         // Integrate as usual
//         p[id].vel.xyz = p[id].vel.xyz + acceleration.xyz * dt;
//         p[id].pos.xyz = p[id].pos.xyz + (p[id].vel.xyz + p2[id].vel.xyz) * (dt / 2.0);
//         p[id].pos.w = p[id].pos.w - dt;
//     }
...
void main() {
    ...
    // Check if the particle needs to respawn
    if (p2[id].pos.w < 0.0) {
        // Respawn at emitter position with new velocity
        vec3 newPos = vec3(emitterX, emitterY, emitterZ);
        vec3 newVel = vec3(
            rand(-15, 15, id * 7.0),  
            10.0,  // Initial upward velocity
            rand(-15, 15, id * 11.0)
        );
        
        // Update both buffers consistently
        p[id].pos.xyz = newPos;
        p2[id].pos.xyz = newPos;
        
        p[id].vel.xyz = newVel;
        p2[id].vel.xyz = newVel;
        
        p[id].pos.w = maxLifeTime;
        p2[id].pos.w = maxLifeTime;
    } else {
        // Update velocity first (using previous velocity)
        p[id].vel.xyz = p2[id].vel.xyz + acceleration.xyz * dt;
        
        // Then update position (using average velocity for Verlet integration)
    ...
    // Color based on particle lifetime
    float lifeRatio = p[id].pos.w / maxLifeTime;
    vec3 white = vec3(1.0, 1.0, 1.0);
    vec3 red = vec3(1.0, 0.0, 0.0);
    vec3 yellow = vec3(1.0, 1.0, 0.0);
